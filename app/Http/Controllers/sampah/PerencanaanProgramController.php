...
class perencanaanprogramController extends Controller
{
    public function index(Request $request)
    {
        $query = perencanaanprogram::query();
        ...
        $perencanaanprogramData = $query
            ->orderBy($sort, $direction)
            ->paginate($request->get('per_page', 10));

        // If AJAX request, return table partial
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.perencanaan-program._table', compact('perencanaanprogramData'))->render();
        }

        return view('admin.laporan-lpj.bidang.perencanaan-program.index', compact('perencanaanprogramData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.perencanaan-program.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('perencanaan-program/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('perencanaan-program/dokumen_lpj', 'public');
            }
            $data['dokumen_lpj'] = $dokumenPaths;
        }

        perencanaanprogram::create($data);

        return redirect()->route('admin.laporan-lpj.bidang.perencanaan-program.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(perencanaanprogram $perencanaanprogram)
    {
        return view('admin.laporan-lpj.bidang.perencanaan-program.show', compact('perencanaanprogram'));
    }

    public function edit(perencanaanprogram $perencanaanprogram)
    {
        ...
        return view('admin.laporan-lpj.bidang.perencanaan-program.edit', compact('perencanaanprogram'));
    }

    public function update(Request $request, perencanaanprogram $perencanaanprogram)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($perencanaanprogram->foto_jurnal) {
                foreach ($perencanaanprogram->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($perencanaanprogram->dokumen_lpj) {
                foreach ($perencanaanprogram->dokumen_lpj as $oldDokumen) {
        ...
        $perencanaanprogram->update($data);

        return redirect()->route('admin.laporan-lpj.bidang.perencanaan-program.index')
                         ->with('OK', 'Data sumber daya berhasil diperbarui.');
    }

    public function destroy(perencanaanprogram $perencanaanprogram)
    {
        ...
        if ($perencanaanprogram->foto_jurnal) {
            foreach ($perencanaanprogram->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($perencanaanprogram->dokumen_lpj) {
            foreach ($perencanaanprogram->dokumen_lpj as $dokumen) {
                Storage::disk('public')->delete($dokumen);
            }
        }

        $perencanaanprogram->delete();
     ...
     */
    public function removeFile(Request $request, perencanaanprogram $perencanaanprogram)
    {
        ...
        $files = $perencanaanprogram->$fileType ?? [];
        ...
        // Update the model
        $perencanaanprogram->update([$fileType => $files]);
