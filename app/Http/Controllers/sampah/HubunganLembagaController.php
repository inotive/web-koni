...
class hubunganlembagaController extends Controller
{
    public function index(Request $request)
    {
        $query = hubunganlembaga::query();
        ...
        $hubunganlembagaData = $query
            ->orderBy($sort, $direction)
            ->paginate($request->get('per_page', 10));

        // If AJAX request, return table partial
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.hubungan-antar-lembaga._table', compact('hubunganlembagaData'))->render();
        }

        return view('admin.laporan-lpj.bidang.hubungan-antar-lembaga.index', compact('hubunganlembagaData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.hubungan-antar-lembaga.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('hubungan-antar-lembaga/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('hubungan-antar-lembaga/dokumen_lpj', 'public');
            }
            $data['dokumen_lpj'] = $dokumenPaths;
        }

        hubunganlembaga::create($data);

        return redirect()->route('admin.laporan-lpj.bidang.hubungan-antar-lembaga.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(hubunganlembaga $hubunganlembaga)
    {
        return view('admin.laporan-lpj.bidang.hubungan-antar-lembaga.show', compact('hubunganlembaga'));
    }

    public function edit(hubunganlembaga $hubunganlembaga)
    {
        ...
        return view('admin.laporan-lpj.bidang.hubungan-antar-lembaga.edit', compact('hubunganlembaga'));
    }

    public function update(Request $request, hubunganlembaga $hubunganlembaga)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($hubunganlembaga->foto_jurnal) {
                foreach ($hubunganlembaga->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($hubunganlembaga->dokumen_lpj) {
                foreach ($hubunganlembaga->dokumen_lpj as $oldDokumen) {
        ...
        $hubunganlembaga->update($data);

        return redirect()->route('admin.laporan-lpj.bidang.hubungan-antar-lembaga.index')
                         ->with('OK', 'Data sumber daya berhasil diperbarui.');
    }

    public function destroy(hubunganlembaga $hubunganlembaga)
    {
        ...
        if ($hubunganlembaga->foto_jurnal) {
            foreach ($hubunganlembaga->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($hubunganlembaga->dokumen_lpj) {
            foreach ($hubunganlembaga->dokumen_lpj as $dokumen) {
                Storage::disk('public')->delete($dokumen);
            }
        }

        $hubunganlembaga->delete();
     ...
     */
    public function removeFile(Request $request, hubunganlembaga $hubunganlembaga)
    {
        ...
        $files = $hubunganlembaga->$fileType ?? [];
        ...
        // Update the model
        $hubunganlembaga->update([$fileType => $files]);
