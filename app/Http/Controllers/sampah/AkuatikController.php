...
use App\Models\akuatik;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class akuatikController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.prestasi.terukur.akuatik._table', compact('akuatikData'))->render();
        }

        return view('admin.laporan-lpj.bidang.prestasi.terukur.akuatik.index', compact('akuatikData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.prestasi.terukur.akuatik.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('akuatik/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('akuatik/dokumen_lpj', 'public');
            }
        ...
        return redirect()->route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.akuatik.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(akuatik $akuatik)
    {
        return view('admin.laporan-lpj.bidang.prestasi.terukur.akuatik.show', compact('akuatik'));
    }

    public function edit(akuatik $akuatik)
    {
        ...
        return view('admin.laporan-lpj.bidang.prestasi.terukur.akuatik.edit', compact('akuatik'));
    }

    public function update(Request $request, akuatik $akuatik)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($akuatik->foto_jurnal) {
                foreach ($akuatik->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($akuatik->dokumen_lpj) {
                foreach ($akuatik->dokumen_lpj as $oldDokumen) {
    ...
    public function destroy(akuatik $akuatik)
    {
        ...
        if ($akuatik->foto_jurnal) {
            foreach ($akuatik->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($akuatik->dokumen_lpj) {
            foreach ($akuatik->dokumen_lpj as $dokumen) {
     ...
     */
    public function removeFile(Request $request, akuatik $akuatik)
    {
