...
class organisasiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $organisasiData = $query
            ->orderBy($sort, $direction)
            ->paginate($request->get('per_page', 10));

        // If AJAX request, return table partial
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.organisasi._table', compact('organisasiData'))->render();
        }

        return view('admin.laporan-lpj.bidang.organisasi.index', compact('organisasiData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.organisasi.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('organisasi/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('organisasi/dokumen_lpj', 'public');
            }
        ...
        return redirect()->route('admin.laporan-lpj.bidang.organisasi.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(organisasi $organisasi)
    {
        return view('admin.laporan-lpj.bidang.organisasi.show', compact('organisasi'));
    }
    ...
    public function edit(organisasi $organisasi)
    {
        ...
        return view('admin.laporan-lpj.bidang.organisasi.edit', compact('organisasi'));
    }
    ...
    public function update(Request $request, organisasi $organisasi)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($organisasi->foto_jurnal) {
                foreach ($organisasi->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($organisasi->dokumen_lpj) {
                foreach ($organisasi->dokumen_lpj as $oldDokumen) {
    ...
    public function destroy(organisasi $organisasi)
    {
        ...
        if ($organisasi->foto_jurnal) {
            foreach ($organisasi->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($organisasi->dokumen_lpj) {
            foreach ($organisasi->dokumen_lpj as $dokumen) {
     ...
     */
    public function removeFile(Request $request, organisasi $organisasi)
    {
        ...
        // Update the model
        $organisasi->update([$fileType => $files]);
