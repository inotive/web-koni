...
class SportScienceController extends Controller
{
    public function index(Request $request)
    {
        $query = sportscience::query();
        ...
        $sportscienceData = $query
            ->orderBy($sort, $direction)
            ->paginate($request->get('per_page', 10));

        // If AJAX request, return table partial
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.sportscience._table', compact('sportscienceData'))->render();
        }

        return view('admin.laporan-lpj.bidang.sportscience.index', compact('sportscienceData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.sportscience.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('sportscience/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('sportscience/dokumen_lpj', 'public');
            }
            $data['dokumen_lpj'] = $dokumenPaths;
        }

        sportscience::create($data);

        return redirect()->route('admin.laporan-lpj.bidang.sport-science.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(sportscience $sportscience)
    {
        return view('admin.laporan-lpj.bidang.sportscience.show', compact('sportscience'));
    }

    public function edit(sportscience $sportscience)
    {
        ...
        return view('admin.laporan-lpj.bidang.sportscience.edit', compact('sportscience'));
    }

    public function update(Request $request, sportscience $sportscience)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($sportscience->foto_jurnal) {
                foreach ($sportscience->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($sportscience->dokumen_lpj) {
                foreach ($sportscience->dokumen_lpj as $oldDokumen) {
        ...
        $sportscience->update($data);

        return redirect()->route('admin.laporan-lpj.bidang.sport-science.index')
                         ->with('OK', 'Data sumber daya berhasil diperbarui.');
    }

    public function destroy(sportscience $sportscience)
    {
        ...
        if ($sportscience->foto_jurnal) {
            foreach ($sportscience->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($sportscience->dokumen_lpj) {
            foreach ($sportscience->dokumen_lpj as $dokumen) {
     ...
     */
    public function removeFile(Request $request, sportscience $sportscience)
    {
        ...
        // Update the model
        $sportscience->update([$fileType => $files]);
