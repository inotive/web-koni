...
class pembinaanhukumController extends Controller
{
    public function index(Request $request)
    {
        $query = pembinaanhukum::query();
        ...
        $pembinaanhukumData = $query
            ->orderBy($sort, $direction)
            ->paginate($request->get('per_page', 10));

        // If AJAX request, return table partial
        if ($request->ajax()) {
            return view('admin.laporan-lpj.bidang.pembinaan-hukum._table', compact('pembinaanhukumData'))->render();
        }

        return view('admin.laporan-lpj.bidang.pembinaan-hukum.index', compact('pembinaanhukumData'));
    }

    public function create()
    {
        return view('admin.laporan-lpj.bidang.pembinaan-hukum.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            foreach ($request->file('foto_jurnal') as $file) {
                $fotoPaths[] = $file->store('pembinaan-hukum/foto_jurnal', 'public');
            }
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            foreach ($request->file('dokumen_lpj') as $file) {
                $dokumenPaths[] = $file->store('pembinaan-hukum/dokumen_lpj', 'public');
            }
            $data['dokumen_lpj'] = $dokumenPaths;
        }

        pembinaanhukum::create($data);

        return redirect()->route('admin.laporan-lpj.bidang.pembinaan-hukum.index')
                         ->with('OK', 'Data sumber daya berhasil ditambahkan.');
    }

    public function show(pembinaanhukum $pembinaanhukum)
    {
        return view('admin.laporan-lpj.bidang.pembinaan-hukum.show', compact('pembinaanhukum'));
    }

    public function edit(pembinaanhukum $pembinaanhukum)
    {
        ...
        return view('admin.laporan-lpj.bidang.pembinaan-hukum.edit', compact('pembinaanhukum'));
    }

    public function update(Request $request, pembinaanhukum $pembinaanhukum)
    {
        ...
        if ($request->hasFile('foto_jurnal')) {
            ...
            if ($pembinaanhukum->foto_jurnal) {
                foreach ($pembinaanhukum->foto_jurnal as $oldFoto) {
        ...
        if ($request->hasFile('dokumen_lpj')) {
            ...
            if ($pembinaanhukum->dokumen_lpj) {
                foreach ($pembinaanhukum->dokumen_lpj as $oldDokumen) {
        ...
        $pembinaanhukum->update($data);

        return redirect()->route('admin.laporan-lpj.bidang.pembinaan-hukum.index')
                         ->with('OK', 'Data sumber daya berhasil diperbarui.');
    }

    public function destroy(pembinaanhukum $pembinaanhukum)
    {
        ...
        if ($pembinaanhukum->foto_jurnal) {
            foreach ($pembinaanhukum->foto_jurnal as $foto) {
                Storage::disk('public')->delete($foto);
            }
        }

        if ($pembinaanhukum->dokumen_lpj) {
            foreach ($pembinaanhukum->dokumen_lpj as $dokumen) {
                Storage::disk('public')->delete($dokumen);
            }
        }

        $pembinaanhukum->delete();
     ...
     */
    public function removeFile(Request $request, pembinaanhukum $pembinaanhukum)
    {
        ...
        // Update the model
        $pembinaanhukum->update([$fileType => $files]);
