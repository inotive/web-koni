...
<div class="container mt-4">
    <div class="card card-form">
        <div class="card-body p-4 p-md-5">
            ...
            [cite_start]<form action="{{ route('admin.konfigurasi.pelatih.update', $pelatih->id) }}" method="POST" enctype="multipart/form-data"> [cite: 580]
                @csrf
                ...
                $fields = [
                    ...
                    'ketersediaan' => ['label' => 'Ketersediaan', 'type' => 'select-static', 'options' => ['Tersedia', 'Tidak Tersedia']], // This field is for design purposes
                ...
                <div class="row align-items-center mb-3">
                    ...
                    <div class="col-md-9">
                        ...
                        @elseif ($field['type'] === 'textarea')
                            <textarea name="{{ $key }}" id="{{ $key }}" class="form-control @error($key) is-invalid @enderror" placeholder="{{ $field['placeholder'] }}" rows="3">{{ $value }}</textarea>
                        @else
                            <input type="{{ $field['type'] }}" name="{{ $key }}" id="{{ $key }}" class="form-control @error($key) is-invalid @enderror" placeholder="{{ $field['placeholder'] ?? '' }}" value="{{ $value }}">
                        @endif
...
<script>
    document.getElementById('foto').addEventListener('change', function(e) {
        const fileName = e.target.files[0] ? e.target.files[0].name : 'Kosongkan jika tidak ingin mengubah foto';
