...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($pembinaanhukumData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($pembinaanhukumData->currentPage() - 1) * $pembinaanhukumData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.pembinaan-hukum.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($pembinaanhukumData) && method_exists($pembinaanhukumData, 'hasPages') && $pembinaanhukumData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $pembinaanhukumData->firstItem() }}-{{ $pembinaanhukumData->lastItem() }} of
                        {{ $pembinaanhukumData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($pembinaanhukumData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $pembinaanhukumData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $pembinaanhukumData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($pembinaanhukumData->hasMorePages())
                            <a href="{{ $pembinaanhukumData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($pembinaanhukumData) && method_exists($pembinaanhukumData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $pembinaanhukumData->count() }} of {{ $pembinaanhukumData->total() }}
                </div>
            @endif
        </div>
    </div>
