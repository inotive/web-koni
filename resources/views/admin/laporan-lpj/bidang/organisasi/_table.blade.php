...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($organisasiData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($organisasiData->currentPage() - 1) * $organisasiData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.organisasi.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($organisasiData) && method_exists($organisasiData, 'hasPages') && $organisasiData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $organisasiData->firstItem() }}-{{ $organisasiData->lastItem() }} of
                        {{ $organisasiData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($organisasiData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $organisasiData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $organisasiData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($organisasiData->hasMorePages())
                            <a href="{{ $organisasiData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($organisasiData) && method_exists($organisasiData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $organisasiData->count() }} of {{ $organisasiData->total() }}
                </div>
            @endif
        </div>
    </div>
