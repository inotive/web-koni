@if ($perencanaanprogramData->isEmpty())
    {{-- Empty State --}}
...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($perencanaanprogramData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($perencanaanprogramData->currentPage() - 1) * $perencanaanprogramData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.perencanaan-program.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($perencanaanprogramData) && method_exists($perencanaanprogramData, 'hasPages') && $perencanaanprogramData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $perencanaanprogramData->firstItem() }}-{{ $perencanaanprogramData->lastItem() }} of
                        {{ $perencanaanprogramData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($perencanaanprogramData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $perencanaanprogramData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $perencanaanprogramData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($perencanaanprogramData->hasMorePages())
                            <a href="{{ $perencanaanprogramData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($perencanaanprogramData) && method_exists($perencanaanprogramData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $perencanaanprogramData->count() }} of {{ $perencanaanprogramData->total() }}
                </div>
            @endif
        </div>
    </div>
