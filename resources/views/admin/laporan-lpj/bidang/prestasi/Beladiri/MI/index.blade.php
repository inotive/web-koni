...
@section('currentSection', 'MI - Muaythai')
...
@section('content')
    ...
    <div class="d-flex flex-column mb-8">
        <h1 class="text-dark fw-bold mb-1">Laporan MI - Muaythai</h1>
    </div>
    ...
    <div class="row col-12 mt-5">
        <div class="card">
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap py-5">
                <h3 class="card-title fw-bold fs-4 mb-0">Daftar MI - Muaythai 2025</h3>

                {{-- Action Buttons --}}
                <div class="d-flex align-items-center gap-2 flex-wrap ms-auto">
                    {{-- Add Button with Access Control --}}
                    @if(auth()->user()->hasRole('superadmin'))
                        <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-beladiri.mi.create') }}" class="btn custom-red-button"
                        ...
                        <div class="dropdown-menu p-3 shadow" style="min-width: 320px;">
                            <div class="mb-3">
                                ...
                                <select id="filter-jenis-kegiatan" class="form-select">
                                    ...
                                    @foreach ($miData->pluck('nama_kegiatan')->unique()->filter() as $kegiatan)
            ...
            <div class="card-body position-relative">
                ...
                <div id="table-container">
                    @include('admin.laporan-lpj.bidang.prestasi.beladiri.mi._table')
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script>
        ...
        function showDetailModal(mi) {
            ...
            if (mi.foto_jurnal && mi.foto_jurnal.length > 0) {
                fotoHtml = `
                    <div class="row g-3">
                        ${mi.foto_jurnal.map(f => `
            ...
            if (mi.dokumen_lpj && mi.dokumen_lpj.length > 0) {
                dokumenHtml = `
                    <div class="d-flex flex-column gap-2">
                        ${mi.dokumen_lpj.map(d => {
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="bg-light p-3 rounded">
                                <div class="mb-2">
                                    ...
                                    <p class="mb-0 text-dark">${mi.nama_program}</p>
                                    <small class="text-muted">${mi.nama_kegiatan}</small>
                                </div>
                                ${mi.tempat_kegiatan ? `
                                    <div class="mb-2">
                                        <label class="fw-semibold text-dark mb-1">Tempat Kegiatan:</label>
                                        <p class="mb-0 text-dark">${mi.tempat_kegiatan}</p>
                                    </div>
                                ` : ''}
                                ${mi.tanggal_kegiatan ? `
                                    <div>
                                        <label class="fw-semibold text-dark mb-1">Tanggal Kegiatan:</label>
                                        <p class="mb-0 text-dark">${new Date(mi.tanggal_kegiatan).toLocaleDateString('id-ID')}</p>
                                    </div>
                                ` : ''}
                            </div>
                        </div>
                                ...
                                <div class="bg-light p-3 rounded">
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <p class="mb-0 text-success fs-5 fw-bold">${formatRupiah(mi.jumlah_anggaran)}</p>
                                        </div>
                                        ${mi.jumlah_realisasi ? `
                                            <div class="col-md-6">
                                                <label class="fw-semibold text-dark mb-1">Realisasi:</label>
                                                <p class="mb-0 text-info fs-5 fw-bold">${formatRupiah(mi.jumlah_realisasi)}</p>
                                            </div>
                                        ` : ''}
                                    </div>
                                </div>
                        ...
                        ${mi.sumber_dana ? `
                            <div class="mb-4">
                                ...
                                <div class="bg-light p-3 rounded">
                                    <p class="mb-0 text-dark">${mi.sumber_dana}</p>
                                </div>
                            </div>
                        ` : ''}
                        ...
                        ${mi.keterangan ? `
                            <div class="mb-2">
                                ...
                                <div class="bg-light p-3 rounded">
                                    <p class="mb-0 text-dark">${mi.keterangan}</p>
                                </div>
                            </div>
                        ` : ''}
...
$(document).ready(function() {
        ...
        $('#submitBtnDelete').on('click', function() {
            ...
            const url = "{{ route('admin.laporan-lpj.bidang.prestasi.cabor-beladiri.mi.destroy', ':id') }}".replace(':id', deleteId);
        ...
        function deleteItemWithSwal(itemId, itemName = 'item ini') {
            const url = "{{ route('admin.laporan-lpj.bidang.prestasi.cabor-beladiri.mi.destroy', ':id') }}".replace(':id', itemId);
