...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($IPSIData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($IPSIData->currentPage() - 1) * $IPSIData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-beladiri.IPSI.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($IPSIData) && method_exists($IPSIData, 'hasPages') && $IPSIData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $IPSIData->firstItem() }}-{{ $IPSIData->lastItem() }} of
                        {{ $IPSIData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($IPSIData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $IPSIData->appends(request()->query())->previousPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Previous">←</a>
                        @endif

                        @php
                            $current = $IPSIData->currentPage();
                            $total = $IPSIData->lastPage();
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $IPSIData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($IPSIData->hasMorePages())
                            <a href="{{ $IPSIData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($IPSIData) && method_exists($IPSIData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $IPSIData->count() }} of {{ $IPSIData->total() }}
                </div>
            @endif
        </div>
    </div>
