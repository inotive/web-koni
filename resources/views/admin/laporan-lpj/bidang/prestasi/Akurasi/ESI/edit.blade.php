...
@section('pageTitle', 'Edit ESI - Esports')
...
@section('subSection4', 'ESI - Esports')
@section('subSection4Url', route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.esi.index'))
@section('currentSection', 'Edit ESI - Esports')
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4" style="padding: 20px 20px">
        <h3 class="fw-bold fs-2 mb-0 text-dark">Edit Data ESI - Esports</h3>
    </div>
            ...
            <div class="row">
                <div class="card card-form">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.esi.update', $esi->id) }}" method="POST"
                            ...
                            @foreach ($fields as $key => $field)
                                @if($key === 'keterangan_tambahan')
                                    ...
                                    @include('admin.laporan-lpj.bidang.prestasi.akurasi.esi.components.file-upload', [
                                        'name' => 'foto_jurnal',
                                        'label' => 'Foto Jurnal',
                                        'type' => 'image',
                                        'maxFiles' => 10,
                                    ...
                                    <div class="col-md-9">
                                        @if ($field['type'] === 'textarea')
                                            <textarea name="{{ $key }}" id="{{ $key }}"
                                                ...
                                                rows="3"
                                                {{ $field['required'] ? 'required' : '' }}>{{ old($key, $esi->$key) }}</textarea>
                                        @else
                                            <input type="{{ $field['type'] }}" name="{{ $key }}"
                                                id="{{ $key }}"
                                                class="form-control @error($key) is-invalid @enderror"
                                                placeholder="{{ $field['placeholder'] ?? '' }}"
                                                value="{{ old($key, $esi->$key) }}"
                            ...
                            <div class="row mt-4">
                                <div class="col-md-9 offset-md-3 d-flex justify-content-between">
                                    ...
                                    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.esi.index') }}"
    ...
    @include('admin.laporan-lpj.bidang.prestasi.akurasi.esi.components.file-upload-scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            initFileUpload({
                ...
                existingFiles: @json($esi->foto_jurnal ?? [])
            });
            ...
            initFileUpload({
                ...
                maxFiles: 10,
                maxSize: 10 * 1024 * 1024,
                existingFiles: @json($esi->dokumen_lpj ?? [])
            });
        });
    </script>
@endsection
