...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($perbaikinData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($perbaikinData->currentPage() - 1) * $perbaikinData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.perbaikin.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($perbaikinData) && method_exists($perbaikinData, 'hasPages') && $perbaikinData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $perbaikinData->firstItem() }}-{{ $perbaikinData->lastItem() }} of
                        {{ $perbaikinData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($perbaikinData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $perbaikinData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $perbaikinData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($perbaikinData->hasMorePages())
                            <a href="{{ $perbaikinData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($perbaikinData) && method_exists($perbaikinData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $perbaikinData->count() }} of {{ $perbaikinData->total() }}
                </div>
            @endif
        </div>
    </div>
