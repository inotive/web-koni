...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($FASIData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($FASIData->currentPage() - 1) * $FASIData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.FASI.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($FASIData) && method_exists($FASIData, 'hasPages') && $FASIData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $FASIData->firstItem() }}-{{ $FASIData->lastItem() }} of
                        {{ $FASIData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($FASIData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $FASIData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $FASIData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($FASIData->hasMorePages())
                            <a href="{{ $FASIData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($FASIData) && method_exists($FASIData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $FASIData->count() }} of {{ $FASIData->total() }}
                </div>
            @endif
        </div>
    </div>
