...
@section('pageTitle', 'Edit PERSANI - Senam')
...
@section('currentSection', 'Edit PERSANI - Senam')
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4" style="padding: 20px 20px">
        <h3 class="fw-bold fs-2 mb-0 text-dark">Edit Data PERSANI - Senam</h3>
    </div>
            ...
            <div class="row">
                <div class="card card-form">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-akurasi.persani.update', $persani->id) }}" method="POST"
                            ...
                            @foreach ($fields as $key => $field)
                                @if($key === 'keterangan_tambahan')
                                    ...
                                    @include('admin.laporan-lpj.bidang.prestasi.akurasi.persani.components.file-upload', [
                                        'name' => 'foto_jurnal',
                                        'label' => 'Foto Jurnal',
                                        'type' => 'image',
                                        'maxFiles' => 10,
                                        'maxSize' => 10,
                                        'existingFiles' => $persani->foto_jurnal ?? [],
                                        'accept' => 'image/*',
                                    ...
                                    @include('admin.laporan-lpj.bidang.prestasi.akurasi.persani.components.file-upload', [
                                        ...
                                        'maxFiles' => 10,
                                        'maxSize' => 10,
                                        'existingFiles' => $persani->dokumen_lpj ?? [],
                                    ...
                                    <div class="col-md-9">
                                        @if ($field['type'] === 'textarea')
                                            <textarea name="{{ $key }}" id="{{ $key }}"
                                                ...
                                                rows="3"
                                                {{ $field['required'] ? 'required' : '' }}>{{ old($key, $persani->$key) }}</textarea>
                                        @else
                                            <input type="{{ $field['type'] }}" name="{{ $key }}"
                                                id="{{ $key }}"
                                                class="form-control @error($key) is-invalid @enderror"
                                                placeholder="{{ $field['placeholder'] ?? '' }}"
                                                value="{{ old($key, $persani->$key) }}"
    ...
    @include('admin.laporan-lpj.bidang.prestasi.akurasi.persani.components.file-upload-scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            initFileUpload({
                ...
                existingFiles: @json($persani->foto_jurnal ?? [])
            });
            ...
            initFileUpload({
                ...
                maxFiles: 10,
                maxSize: 10 * 1024 * 1024,
                existingFiles: @json($persani->dokumen_lpj ?? [])
            });
        });
    </script>
@endsection
