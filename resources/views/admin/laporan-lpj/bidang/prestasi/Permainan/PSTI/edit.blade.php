...
@section('pageTitle', 'Edit PSTI - Sepak Takraw')
...
@section('subSection4', 'PSTI - Sepak Takraw')
@section('subSection4Url', route('admin.laporan-lpj.bidang.prestasi.cabor-permainan.psti.index'))
@section('currentSection', 'Edit PSTI - Sepak Takraw')
...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4" style="padding: 20px 20px">
        <h3 class="fw-bold fs-2 mb-0 text-dark">Edit Data PSTI - Sepak Takraw</h3>
    </div>
            ...
            <div class="row">
                <div class="card card-form">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-permainan.psti.update', $psti->id) }}" method="POST"
                            ...
                            @foreach ($fields as $key => $field)
                                @if($key === 'keterangan_tambahan')
                                    ...
                                    @include('admin.laporan-lpj.bidang.prestasi.permainan.psti.components.file-upload', [
                                        'name' => 'foto_jurnal',
                                        'label' => 'Foto Jurnal',
                                        'type' => 'image',
                                        'maxFiles' => 10,
                                        'maxSize' => 10,
                                        'existingFiles' => $psti->foto_jurnal ?? [],
                                        'accept' => 'image/*',
                                    ...
                                    @include('admin.laporan-lpj.bidang.prestasi.permainan.psti.components.file-upload', [
                                        ...
                                        'maxFiles' => 10,
                                        'maxSize' => 10,
                                        'existingFiles' => $psti->dokumen_lpj ?? [],
                                    ...
                                    <div class="col-md-9">
                                        @if ($field['type'] === 'textarea')
                                            <textarea name="{{ $key }}" id="{{ $key }}"
                                                ...
                                                rows="3"
                                                {{ $field['required'] ? 'required' : '' }}>{{ old($key, $psti->$key) }}</textarea>
                                        @else
                                            <input type="{{ $field['type'] }}" name="{{ $key }}"
                                                id="{{ $key }}"
                                                class="form-control @error($key) is-invalid @enderror"
                                                placeholder="{{ $field['placeholder'] ?? '' }}"
                                                value="{{ old($key, $psti->$key) }}"
                            ...
                            <div class="row mt-4">
                                <div class="col-md-9 offset-md-3 d-flex justify-content-between">
                                    ...
                                    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-permainan.psti.index') }}"
    ...
    @include('admin.laporan-lpj.bidang.prestasi.permainan.psti.components.file-upload-scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            initFileUpload({
                ...
                existingFiles: @json($psti->foto_jurnal ?? [])
            });
            ...
            initFileUpload({
                ...
                maxFiles: 10,
                maxSize: 10 * 1024 * 1024,
                existingFiles: @json($psti->dokumen_lpj ?? [])
            });
        });
    </script>
@endsection
