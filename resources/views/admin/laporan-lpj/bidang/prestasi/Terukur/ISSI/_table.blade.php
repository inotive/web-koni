...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($issiData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($issiData->currentPage() - 1) * $issiData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.issi.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            <div class="mb-2 mb-md-0">
                <div class="d-flex align-items-center">
                    ...
                    <select name="per_page" class="form-select form-select-sm w-auto">
                        @foreach ([10, 25, 50, 100] as $lissit)
                            <option value="{{ $lissit }}"
                                {{ request('per_page', 10) == $lissit ? 'selected' : '' }}>
                                {{ $lissit }}
                            </option>
                        @endforeach
                    </select>
            ...
            @if (isset($issiData) && method_exists($issiData, 'hasPages') && $issiData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $issiData->firstItem() }}-{{ $issiData->lastItem() }} of
                        {{ $issiData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($issiData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $issiData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $issiData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($issiData->hasMorePages())
                            <a href="{{ $issiData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($issiData) && method_exists($issiData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $issiData->count() }} of {{ $issiData->total() }}
                </div>
            @endif
        </div>
    </div>
