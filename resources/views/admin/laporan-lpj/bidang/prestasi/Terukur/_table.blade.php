<div class="row g-10">
    {{-- Sample Cabor Terukur folders based on the screenshot --}}
    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.POSISI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="Posisi - Dayung">
                    Posisi - Dayung
                </h5>
                <p>16 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.IMI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="IMI - Balap Motor">
                    IMI - Balap Motor
                </h5>
                <p>6 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.akuatik.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="Akuatik - Renang">
                    Akuatik - Renang
                </h5>
                <p>10 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.FPTI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="FPTI - Panijat Tebing">
                    FPTI - Panijat Tebing
                </h5>
                <p>5 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.PASI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="PASI - Atletik">
                    PASI - Atletik
                </h5>
                <p>10 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.PABERSI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="PABERSI - Angkat Berat">
                    PABERSI - Angkat Berat
                </h5>
                <p>11 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.PARSI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="PARSI - Angkat Besi">
                    PARSI - Angkat Besi
                </h5>
                <p>16 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.ISSI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="ISSI - Balap Sepeda">
                    ISSI - Balap Sepeda
                </h5>
                <p>10 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.FAJI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="FAJI - Arung Jeram">
                    FAJI - Arung Jeram
                </h5>
                <p>10 Dokumen</p>
            </div>
        </div>
    </a>

    <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.PORSEROSI.index') }}" class="col-12 col-sm-6 col-md-3">
        <div class="card gap-2 p-3 text-center shadow-sm" style="min-height: 160px; transition: background-color 0.3s;"
            onmouseover="this.style.backgroundColor='#E5E7EB'" onmouseout="this.style.backgroundColor='#ffffff'">
            <i class="ki-outline ki-folder text-gray-600" style="font-size: 80px"></i>
            <div class="text-center">
                <h5 class="d-inline-block text-truncate w-100 mb-0" style="max-width: 200px;" title="PORSEROSI - Sepatu Roda">
                    PORSEROSI - Sepatu Roda
                </h5>
                <p>15 Dokumen</p>
            </div>
        </div>
    </a>
</div>
