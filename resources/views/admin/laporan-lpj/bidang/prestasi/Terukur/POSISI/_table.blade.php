...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($posisiData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($posisiData->currentPage() - 1) * $posisiData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.posisi.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            <div class="mb-2 mb-md-0">
                <div class="d-flex align-items-center">
                    ...
                    <select name="per_page" class="form-select form-select-sm w-auto">
                        @foreach ([10, 25, 50, 100] as $lposisit)
                            <option value="{{ $lposisit }}"
                                {{ request('per_page', 10) == $lposisit ? 'selected' : '' }}>
                                {{ $lposisit }}
                            </option>
                        @endforeach
                    </select>
            ...
            @if (isset($posisiData) && method_exists($posisiData, 'hasPages') && $posisiData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $posisiData->firstItem() }}-{{ $posisiData->lastItem() }} of
                        {{ $posisiData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($posisiData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $posisiData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $posisiData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($posisiData->hasMorePages())
                            <a href="{{ $posisiData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($posisiData) && method_exists($posisiData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $posisiData->count() }} of {{ $posisiData->total() }}
                </div>
            @endif
        </div>
    </div>
