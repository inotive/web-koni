...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($akuatikData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($akuatikData->currentPage() - 1) * $akuatikData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.akuatik.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($akuatikData) && method_exists($akuatikData, 'hasPages') && $akuatikData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $akuatikData->firstItem() }}-{{ $akuatikData->lastItem() }} of
                        {{ $akuatikData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($akuatikData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $akuatikData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $akuatikData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($akuatikData->hasMorePages())
                            <a href="{{ $akuatikData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($akuatikData) && method_exists($akuatikData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $akuatikData->count() }} of {{ $akuatikData->total() }}
                </div>
            @endif
        </div>
    </div>
