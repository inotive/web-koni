...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($fajiData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($fajiData->currentPage() - 1) * $fajiData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.prestasi.cabor-terukur.faji.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($fajiData) && method_exists($fajiData, 'hasPages') && $fajiData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $fajiData->firstItem() }}-{{ $fajiData->lastItem() }} of
                        {{ $fajiData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($fajiData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $fajiData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $fajiData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($fajiData->hasMorePages())
                            <a href="{{ $fajiData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($fajiData) && method_exists($fajiData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $fajiData->count() }} of {{ $fajiData->total() }}
                </div>
            @endif
        </div>
    </div>
