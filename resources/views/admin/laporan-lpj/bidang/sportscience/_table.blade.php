...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($sportscienceData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($sportscienceData->currentPage() - 1) * $sportscienceData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.sport-science.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($sportscienceData) && method_exists($sportscienceData, 'hasPages') && $sportscienceData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $sportscienceData->firstItem() }}-{{ $sportscienceData->lastItem() }} of
                        {{ $sportscienceData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($sportscienceData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $sportscienceData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $sportscienceData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($sportscienceData->hasMorePages())
                            <a href="{{ $sportscienceData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($sportscienceData) && method_exists($sportscienceData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $sportscienceData->count() }} of {{ $sportscienceData->total() }}
                </div>
            @endif
        </div>
    </div>
