...
@section('pageTitle', 'Edit Laporan Sport Science')
...
@section('subSection2', 'Sport Science & IPTEK')
@section('subSection2Url', route('admin.laporan-lpj.bidang.sport-science.index'))
@section('currentSection', 'Edit Laporan Sport Science')
...
@section('content')
            ...
            <div class="row">
                <div class="card card-form">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('admin.laporan-lpj.bidang.sport-science.update', $sportscience->id) }}" method="POST"
                            ...
                            @foreach ($fields as $key => $field)
                                ...
                                <div class="row align-items-start mb-4">
                                    ...
                                    <div class="col-md-9">
                                        @if($sportscience->foto_jurnal && count($sportscience->foto_jurnal) > 0)
                                            <div class="current-files">
                                                <h6>Foto saat ini:</h6>
                                                <div id="existingFotoContainer">
                                                    @foreach($sportscience->foto_jurnal as $index => $foto)
                                                        <div class="existing-file-item" data-type="foto" data-index="{{ $index }}">
                                                            <img src="{{ asset('storage/' . $foto) }}" class="existing-preview-image" alt="Current Image">
                                                            <div class="file-info">
                                                                <div class="file-name">{{ basename($foto) }}</div>
                                                                <div class="file-size">File saat ini</div>
                                                            </div>
                                                            <a href="{{ asset('storage/' . $foto) }}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
                                                                <i class="fas fa-eye"></i>
                                                            </a>
                                                            <button type="button" class="remove-file" onclick="removeExistingFile('foto', {{ $index }})">
                                                                <i class="fas fa-times"></i>
                                                            </button>
                                                        </div>
                                                    @endforeach
                                                </div>
                                                <input type="hidden" name="existing_foto_jurnal" id="existingFotoInput" value="{{ json_encode($sportscience->foto_jurnal) }}">
                                            </div>
                                        @endif
                                ...
                                <div class="row align-items-start mb-4">
                                    ...
                                    <div class="col-md-9">
                                        @if($sportscience->dokumen_lpj && count($sportscience->dokumen_lpj) > 0)
                                            <div class="current-files">
                                                <h6>Dokumen saat ini:</h6>
                                                <div id="existingDokumenContainer">
                                                    @foreach($sportscience->dokumen_lpj as $index => $dokumen)
                                                        @php
                                                            $extension = pathinfo($dokumen, PATHINFO_EXTENSION);
                                                        @endphp
                                                        <div class="existing-file-item" data-type="dokumen" data-index="{{ $index }}">
                                                            <div class="existing-file-icon">
                                                                <i class="{{ getFileIconSport($extension) }} {{ getFileColorSport($extension) }}"></i>
                                                            </div>
                                                            <div class="file-info">
                                                                <div class="file-name">{{ basename($dokumen) }}</div>
                                                                <div class="file-size">File saat ini</div>
                                                            </div>
                                                            <a href="{{ asset('storage/' . $dokumen) }}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
                                                                <i class="fas fa-eye"></i>
                                                            </a>
                                                            <button type="button" class="remove-file" onclick="removeExistingFile('dokumen', {{ $index }})">
                                                                <i class="fas fa-times"></i>
                                                            </button>
                                                        </div>
                                                    @endforeach
                                                </div>
                                                <input type="hidden" name="existing_dokumen_lpj" id="existingDokumenInput" value="{{ json_encode($sportscience->dokumen_lpj) }}">
                                            </div>
                                        @endif
                                    ...
                                    <div class="col-md-9">
                                        @if ($field['type'] === 'textarea')
                                            <textarea name="{{ $key }}" id="{{ $key }}"
                                                ...
                                                rows="3"
                                                {{ $field['required'] ? 'required' : '' }}>{{ old($key, $sportscience->$key) }}</textarea>
                                        @else
                                            <input type="{{ $field['type'] }}" name="{{ $key }}"
                                                id="{{ $key }}"
                                                class="form-control @error($key) is-invalid @enderror"
                                                placeholder="{{ $field['placeholder'] ?? '' }}"
                                                value="{{ old($key, $sportscience->$key) }}"
                            ...
                            <div class="row mt-4">
                                <div class="col-md-9 offset-md-3 d-flex justify-content-between">
                                    ...
                                    <a href="{{ route('admin.laporan-lpj.bidang.sport-science.index') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let existingFotoFiles = @json($sportscience->foto_jurnal ?? []);
            let existingDokumenFiles = @json($sportscience->dokumen_lpj ?? []);
