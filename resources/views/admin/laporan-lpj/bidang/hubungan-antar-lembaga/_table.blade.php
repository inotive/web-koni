@if ($hubunganlembagaData->isEmpty())
    {{-- Empty State --}}
...
@else
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle" id="kt_datatable_dom_positioning_sumberdaya">
            ...
            <tbody>
                @forelse ($hubunganlembagaData as $index => $data)
                    <tr>
                        <td class="text-center">
                            {{ ($hubunganlembagaData->currentPage() - 1) * $hubunganlembagaData->perPage() + $index + 1 }}
                        </td>
                        <td>
                            ...
                            <div class="dropdown dropdown-action" data-row-id="{{ $data->id }}">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    ...
                                    @if(auth()->user()->hasRole('superadmin'))
                                        <li>
                                            <a href="{{ route('admin.laporan-lpj.bidang.hubungan-antar-lembaga.edit', $data->id) }}"
    ...
    <div class="table-footer">
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap">
            ...
            @if (isset($hubunganlembagaData) && method_exists($hubunganlembagaData, 'hasPages') && $hubunganlembagaData->hasPages())
                <div class="d-flex align-items-center gap-3">
                    <div class="text-muted small">
                        {{ $hubunganlembagaData->firstItem() }}-{{ $hubunganlembagaData->lastItem() }} of
                        {{ $hubunganlembagaData->total() }}
                    </div>

                    <div class="d-flex align-items-center gap-2">
                        @if ($hubunganlembagaData->onFirstPage())
                            <span class="pagination-arrow disabled">←</span>
                        @else
                            <a href="{{ $hubunganlembagaData->appends(request()->query())->previousPageUrl() }}"
                        ...
                        <div class="d-flex align-items-center">
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $hubunganlembagaData->appends(request()->query())->url($i) }}"
                                       class="pagination-number pagination-link">{{ $i }}</a>
                                @endif
                            @endfor
                        </div>

                        @if ($hubunganlembagaData->hasMorePages())
                            <a href="{{ $hubunganlembagaData->appends(request()->query())->nextPageUrl() }}"
                               class="pagination-arrow pagination-link"
                               aria-label="Next">→</a>
                        @else
                            <span class="pagination-arrow disabled">→</span>
            ...
            @elseif(isset($hubunganlembagaData) && method_exists($hubunganlembagaData, 'hasPages'))
                <div class="text-muted small">
                    1-{{ $hubunganlembagaData->count() }} of {{ $hubunganlembagaData->total() }}
                </div>
            @endif
        </div>
    </div>
