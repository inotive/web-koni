            ...
            <div class="row">
                <div class="card card-form">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('admin.laporan-lpj.bidang.mobilisasi-sumberdaya.update', $sumberdaya->id) }}" method="POST"
                            ...
                            <div class="row align-items-start mb-4">
                                ...
                                <div class="col-md-9">
                                    @if($sumberdaya->foto_jurnal && count($sumberdaya->foto_jurnal) > 0)
                                        <div class="current-files" id="currentImageContainer">
                                            <small class="text-muted d-block mb-2">Foto saat ini:</small>
                                            <div class="d-flex flex-wrap gap-2">
                                                @foreach($sumberdaya->foto_jurnal as $foto)
                                                    <div class="d-flex align-items-center">
                                                        <img src="{{ asset('storage/' . $foto) }}" class="preview-image me-2" alt="Current Image">
                                                        <a href="{{ asset('storage/' . $foto) }}" target="_blank" class="text-decoration-none text-primary">
                                                            Lihat foto
                                                        </a>
                                                    </div>
                                                @endforeach
                                        ...
                                        <div class="d-flex align-items-center gap-12">
                                            ...
                                            <div class="flex-grow-1">
                                                <p class="file-upload-text mb-0" id="file-name-display">
                                                    {{ $sumberdaya->foto_jurnal && count($sumberdaya->foto_jurnal) > 0 ? 'Klik untuk mengganti foto' : 'Seret dan lepas file di sini, atau klik untuk mengunggah.' }}
                                                </p>
                            ...
                            <div class="row align-items-start mb-4">
                                ...
                                <div class="col-md-9">
                                    @if($sumberdaya->dokumen_lpj && count($sumberdaya->dokumen_lpj) > 0)
                                        <div class="current-files" id="currentDocumentContainer">
                                            <small class="text-muted d-block mb-2">Dokumen saat ini:</small>
                                            <div class="d-flex flex-wrap gap-2">
                                                @foreach($sumberdaya->dokumen_lpj as $dokumen)
                                                    <div class="d-flex align-items-center">
                                                        <i class="fas fa-file-alt me-2 text-primary" style="font-size: 1.5rem;"></i>
                                                        <a href="{{ asset('storage/' . $dokumen) }}" target="_blank" class="text-decoration-none text-primary">
                                                            {{ basename($dokumen) }}
                                                        </a>
                                                    </div>
                                                @endforeach
                                        ...
                                        <div class="d-flex align-items-center gap-12">
                                            ...
                                            <div class="flex-grow-1">
                                                <p class="file-upload-text mb-0" id="dokumen-file-name-display">
                                                    {{ $sumberdaya->dokumen_lpj && count($sumberdaya->dokumen_lpj) > 0 ? 'Klik untuk mengganti dokumen' : 'Seret dan lepas file di sini, atau klik untuk mengunggah.' }}
                                                </p>
                                    ...
                                    <div class="col-md-9">
                                        @if ($field['type'] === 'textarea')
                                            <textarea name="{{ $key }}" id="{{ $key }}"
                                                ...
                                                rows="3"
                                                {{ $field['required'] ? 'required' : '' }}>{{ old($key, $sumberdaya->$key) }}</textarea>
                                        @else
                                            <input type="{{ $field['type'] }}" name="{{ $key }}"
                                                id="{{ $key }}"
                                                class="form-control @error($key) is-invalid @enderror"
                                                placeholder="{{ $field['placeholder'] ?? '' }}"
                                                value="{{ old($key, $sumberdaya->$key) }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            uploadInput.addEventListener('change', function() {
                ...
                if (file) {
                    ...
                    fileNameDisplay.textContent = '{{ $sumberdaya->foto_jurnal && count($sumberdaya->foto_jurnal) > 0 ? "Klik untuk mengganti foto" : "Seret dan lepas file di sini, atau klik untuk mengunggah." }}';
                    previewContainer.innerHTML = '';
                }
            });
            ...
            dokumenUploadInput.addEventListener('change', function() {
                ...
                if (files.length > 0) {
                    ...
                    dokumenFileNameDisplay.textContent = `File baru dipilih: ${files.length} file`;
                    ...
                    if (currentDocumentContainer) {
                        currentDocumentContainer.style.display = 'block';
                    }
                    dokumenFileNameDisplay.textContent = '{{ $sumberdaya->dokumen_lpj && count($sumberdaya->dokumen_lpj) > 0 ? "Klik untuk mengganti dokumen" : "Seret dan lepas file di sini, atau klik untuk mengunggah." }}';
