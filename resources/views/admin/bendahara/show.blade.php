...
@section('pageTitle', 'Detail Laporan Bendahara')
@section('mainSection', 'Main Menu')
@section('subSection', 'Database Bendahara')
                ...
                <div class="col-12 col-xl-10">
                    <div class="card">
                        <div class="card-header">
                            <h3>{{ $laporanBendahara->judul }}</h3>
                        </div>
                        <div class="card-body">
                            <!-- Basic Information -->
                            <div class="info-grid">
                                <h2 class="info-label fw-bold">Dibuat</h2>
                                <span class="info-value fw-bold">{{ $laporanBendahara->created_at->format('d F Y, H:i') }}</span>

                                <h2 class="info-label fw-bold">Diperbarui</h2>
                                <span class="info-value fw-bold">{{ $laporanBendahara->updated_at->format('d F Y, H:i') }}</span>
                            </div>
                            ...
                            @if($laporanBendahara->dokumen)
                                ...
                                <div class="file-section">
                                    <div class="file-header">
                                        <div class="file-icon-circle {{ $fileType }}">
                                            <i class="fas {{ $iconClass }}"></i>
                                        </div>
                                        <div class="file-info">
                                            <h6>{{ $fileName }}</h6>
                                            <p class="file-meta">
                                                {{ strtoupper($fileExtension) }} • {{ $fileSize }} KB •
                                                @if($fileExists)
                                    ...
                                    @if($fileExists)
                                        ...
                                        <div class="preview-container">
                                            ...
                                            <div class="preview-content">
                                                ...
                                                @if($fileExtension === 'pdf')
                                                    <iframe src="{{ asset('storage/' . $laporanBendahara->dokumen) }}"
                                                            class="pdf-viewer"
                                                            title="PDF Preview"
                                                            onload="hideLoading(this)">
                                                    </iframe>
                                                @elseif(in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'bmp']))
                                                    <img src="{{ asset('storage/' . $laporanBendahara->dokumen) }}"
                                                         alt="Image Preview"
                                                         class="image-preview"
                                                         onload="hideLoading(this)"
                                                ...
                                                @else
                                                    <div class="no-preview">
                                                        ...
                                                        <p>File {{ strtoupper($fileExtension) }} tidak dapat ditampilkan sebagai preview</p>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                            ...
                            <div class="btn-group-actions">
                                <a href="{{ route('admin.bendahara.index') }}" class="btn-minimal btn-back">
