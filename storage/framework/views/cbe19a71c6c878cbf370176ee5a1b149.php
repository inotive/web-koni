...
<?php $__env->startSection('script'); ?>
        ...
        return new Promise((resolve, reject) => {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Check if response is JSON (from AJAX) or HTML (from direct request)
                    if (typeof response === 'object' && response.table_html !== undefined) {
                        // Handle JSON response from AJAX
                        $('#table-container').html(response.table_html);
                        
                        // Update the budget information
                        if (response.current_budget !== undefined && response.target_anggaran !== undefined) {
                            updateBudgetDisplay(response.current_budget, response.target_anggaran, response.kegiatan_count, response.target_kegiatan);
                        }
                    } else {
                        // Handle HTML response (fallback for direct requests)
    ...
    function updateBudgetDisplay(current_budget, target_anggaran, kegiatan_count, target_kegiatan) {
        const percentage = (target_anggaran > 0) ? (current_budget / target_anggaran) * 100 : 0;
        
        // Update the budget display elements
        $('.budget-amount').text('Rp ' + current_budget.toLocaleString('id-ID', {minimumFractionDigits: 0, maximumFractionDigits: 0}).replace(/,/g, '.') + ' / Rp ' + target_anggaran.toLocaleString('id-ID', {minimumFractionDigits: 0, maximumFractionDigits: 0}).replace(/,/g, '.'));
        $('.budget-percentage').text(Math.round(percentage) + '%');
        $('.budget-percentage').attr('title', Math.round(percentage, 2) + '% dari total anggaran');
        
        // Update progress bar width
        $('.progress-bar').css('width', percentage + '%');
        $('.progress-bar').attr('aria-valuenow', percentage);
        
        // Update kegiatan count display
        $('.kegiatan-count').text(kegiatan_count + ' Kegiatan Berjalan');
        $('.target-kegiatan-count').text(target_kegiatan + ' Target Kegiatan');
    }
    ...
    $(document).on('change', '#yearFilterSelect', function() {
